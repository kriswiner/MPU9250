 ...
 */
#include "Wire.h"  // not the wire library, can't use pull-ups
...
uint8_t Mscale = MFS_14BITS; // Choose 14-bit or 16-bit magnetometer resolution
...
float   accelbias[3], magCalibration[3], magbias[3];  // Sensitivity adjustment values (calculated biases or factory calibrations)      
...
*/
  calibrateMPU9250(accelbias); // Calibrate gyro and accelerometers, load biases in bias registers

  initMPU9250(); 
  Serial.println("MPU9250 initialized for active data mode...."); // Initialize device for active mode read of acclerometer, gyroscope, and temperature

  // Read the WHO_AM_I register of the magnetometer, this is a good test of communication
  byte c = readByte(AK8963_ADDRESS, WHO_AM_I_AK8963);  // Read WHO_AM_I register for AK8963
  display.clearDisplay();
...
void calibrateMPU9250(float * destination)
{  
  uint8_t data[12]; // data array to hold accelerometer and gyro x, y, z, data
  int8_t abias[6], gbias[6];
...
// At end of sample accumulation, turn off FIFO sensor read
  ...
  for (ii = 0; ii < packet_count; ii++) {
    int8_t accel_cur[3], gyro_cur[3];
    readBytes(MPU9250_ADDRESS, FIFO_R_W, 12, data); // read data for averaging
    accel_bias[0] += (((int16_t)data[0] << 8) | data[1]  ) / packet_count; // Divide sum of FIFO gyro data by number of samples
    accel_bias[1] += (((int16_t)data[2] << 8) | data[3]  ) / packet_count;
    accel_bias[2] += (((int16_t)data[4] << 8) | data[5]  ) / packet_count;
    gyro_bias[0]  += (((int16_t)data[6] << 8) | data[7]  ) / packet_count;
    gyro_bias[1]  += (((int16_t)data[8] << 8) | data[9]  ) / packet_count;
    gyro_bias[2]  += (((int16_t)data[10] << 8) | data[11]) / packet_count;
  } 
...
// Construct the gyro biases for push to the hardware gyro bias registers, which are reset to zero upon device startup
  gbias[0] = (-gyro_bias[0]/4  >> 8) & 0xFF; // Divide by 4 to get 32.9 LSB per deg/s to conform to expected bias input format
  gbias[1] = (-gyro_bias[0]/4)       & 0xFF; // Biases are additive, so change sign on calculated average gyro biases
  gbias[2] = (-gyro_bias[1]/4  >> 8) & 0xFF;
  gbias[3] = (-gyro_bias[1]/4)       & 0xFF;
  gbias[4] = (-gyro_bias[2]/4  >> 8) & 0xFF;
  gbias[5] = (-gyro_bias[2]/4)       & 0xFF;
  
// Push gyro biases to hardware registers
  writeByte(MPU9250_ADDRESS, XG_OFFSET_H, gbias[0]);
  writeByte(MPU9250_ADDRESS, XG_OFFSET_L, gbias[1]);
  writeByte(MPU9250_ADDRESS, YG_OFFSET_H, gbias[2]);
  writeByte(MPU9250_ADDRESS, YG_OFFSET_L, gbias[3]);
  writeByte(MPU9250_ADDRESS, ZG_OFFSET_H, gbias[4]);
  writeByte(MPU9250_ADDRESS, ZG_OFFSET_L, gbias[5]);
...
// the accelerometer biases calculated above must be divided by 8.
  ...
  abias[0] = (accel_bias_reg[0] >> 8) & 0xFF;
  abias[1] = (accel_bias_reg[0])      & 0xFF;
  abias[1] = abias[1] | mask_bit[0]; // preserve temperature compensation bit when writing back to accelerometer bias registers
  abias[2] = (accel_bias_reg[1] >> 8) & 0xFF;
  abias[3] = (accel_bias_reg[1])      & 0xFF;
  abias[3] = abias[3] | mask_bit[1]; // preserve temperature compensation bit when writing back to accelerometer bias registers
  abias[4] = (accel_bias_reg[2] >> 8) & 0xFF;
  abias[5] = (accel_bias_reg[2])      & 0xFF;
  abias[5] = abias[5] | mask_bit[2]; // preserve temperature compensation bit when writing back to accelerometer bias registers
 
// Apparently this is not working for the acceleration biases in the MPU-9250
// Are we handling the temperature correction bit properly?
// Push accelerometer biases to hardware registers
//  writeByte(MPU9250_ADDRESS, XA_OFFSET_H, abias[0]);
//  writeByte(MPU9250_ADDRESS, XA_OFFSET_L, abias[1]);
//  writeByte(MPU9250_ADDRESS, YA_OFFSET_H, abias[2]);
//  writeByte(MPU9250_ADDRESS, YA_OFFSET_L, abias[3]);
//  writeByte(MPU9250_ADDRESS, ZA_OFFSET_H, abias[4]);
//  writeByte(MPU9250_ADDRESS, ZA_OFFSET_L, abias[5]);
